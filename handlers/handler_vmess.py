...
async def vmess_list_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        p = await run_script_and_reply(update, context, ['sudo', '/opt/hokage-bot/list_vmess_users.sh'], "Daftar Akun VMESS")
...
async def renew_vmess_get_duration_and_execute(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await send_script_output(update, context, ['sudo', '/opt/hokage-bot/create_renew_vmess.sh', username, duration])
    context.user_data.clear()
    return states.ROUTE
